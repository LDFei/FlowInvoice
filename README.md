...
- [ ] **#28 短期记忆持久化**：MemorySaver → SqliteSaver（HITL 挂起后服务重启可恢复）—— **#52 已部分关闭**：异步模式持久化 checkpointer（PG/SQLite）落地，跨进程/重启恢复已验证；同步默认仍 MemorySaver（行为不变），如需同步模式也持久化再单开
